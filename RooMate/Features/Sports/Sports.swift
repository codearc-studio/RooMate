...
struct SportsGame: Identifiable, Hashable, Codable {
  ...
  var id: String {
    [rawDateString, team, opponent, location, time]
      .joined(separator: "|")
      .lowercased()
  }
  ...
  init(
    rawDateString: String = "", date: Date? = nil, day: String = "", team: String = "",
    opponent: String = "", location: String = "", time: String = "", dismiss: String = "",
    `return`: String = "", notesRaw: String = ""
  ) {
...
enum SportsHelpers {
  ...
  static func inferStatus(from notes: String) -> SportsGame.Status {
    ...
    if lower.contains("no state tourney") || lower.contains("no state tournament") {
      return .eliminated
    }
