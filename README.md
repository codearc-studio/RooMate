...
RooMate 6.0.4 is the current release source. Its notarized universal archive contains bundle version 6.0.4, build 7, requires macOS 14 or later, and is represented by the matching signed item in `appcast.xml`. The earlier archive labeled RooMate 6.0.1 contains bundle version 6.0, build 4; that historical identity is preserved in the feed so Sparkle compares the actual released bundles correctly.
